...
let marketData = require('./poloniex-markets.json');
...
Trader.prototype.getRawOpenOrders = function(callback) {
  let fetch = next => this.poloniex.returnOpenOrders(this.currency, this.asset, this.processResponse(next));
...
Trader.prototype.getPortfolio = function(callback) {
  ...
  let fetch = next => this.poloniex.myBalances(this.processResponse(next));
...
Trader.prototype.getFullPortfolio = function(callback) {
  let handle = (err, data) => {
    ...
    let asset = data[this.asset];
    let currency = data[this.currency];
  ...
  let fetch = next => this.poloniex.returnCompleteBalances(this.processResponse(next));
  retry(null, fetch, handle);
};
...
Trader.prototype.getTicker = function(callback) {
  ...
  let fetch = next => this.poloniex.getTicker(this.processResponse(next));
  retry(null, fetch, handle);
};
...
Trader.prototype.getFee = function(callback) {
  ...
  let fetch = next => this.poloniex._private('returnFeeInfo', this.processResponse(next));
  retry(null, fetch, handle);
};
...
Trader.prototype.checkOrder = function(id, callback) {
  ...
  let fetch = next => this.poloniex.myOpenOrders(this.currency, this.asset, this.processResponse(next, 'checkOrder'));
  retry(null, fetch, handle);
};
...
Trader.prototype.cancelOrder = function(order, callback) {
  ...
  let fetch = next => this.poloniex.cancelOrder(this.currency, this.asset, order, this.processResponse(next, 'cancelOrder', order));
  retry(null, fetch, handle);
};
